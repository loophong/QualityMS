...
public class PlanApprovalTableController {
    ...
    public R approval(@RequestBody PlanApprovalTableEntity planApprovalTable) {
        ...
        if (planApprovalTable.getApprovalStatus() == ApprovalStatus.APPROVED) {
            ...
            // 发送通知
            messageService.sendMessages(new CreateNoticeParams(ShiroUtils.getUserId(), new Long[]{Long.valueOf(approvalTable.getSubmitter())}, "您的新建计划审批通过", "计划审批通知", "plan_page"));
            ...
            // 发送通知
            messageService.sendMessages(new CreateNoticeParams(ShiroUtils.getUserId(), new Long[]{Long.valueOf(planApprovalTable.getSubmitter())}, "您的新建计划审批未通过", "计划审批通知", "plan_page"));
        }
