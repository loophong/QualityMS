...
<script>
import AddOrUpdate from './qcallcount-add-or-update'
export default {
  data() {
    return {
      dataForm: {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      await this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$emit('allCount', data.page.list[0].qcMemberCount);
        } else {
          this.dataList = []
          this.totalPage = 0
        }
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/qcManagement/qcAllCount/delete'),
          method: 'post',
