...
export default {
  ...
  methods: {
    ...
    async addTemplate() {
      ...
      await this.$http({
        ...
        data: this.$http.adornData({
          ...
          conplanSeries: JSON.stringify(this.nodes),
    ...
    modifyNode() {
      if (this.selectedNode) {
        const newLabel =
          prompt("请输入新的节点标签", this.selectedNode.label) ||
          this.selectedNode.label;
        if (
          !this.nodes.some(
            (node) =>
              node.id !== this.selectedNode.id && node.label === newLabel
          )
        ) {
          const newSize =
            parseFloat(prompt("请输入新的节点大小", this.selectedNode.size)) ||
            this.selectedNode.size;
          const newX =
            parseFloat(prompt("请输入新的X坐标", this.selectedNode.x)) ||
            this.selectedNode.x;
          const newY =
            parseFloat(prompt("请输入新的Y坐标", this.selectedNode.y)) ||
            this.selectedNode.y;
    ...
    modifyLink() {
      const sourceLabel = prompt(
        "请输入当前起始节点标签",
        this.selectedLink ? this.selectedLink.source : ""
      );
      const targetLabel = prompt(
        "请输入当前目标节点标签",
        this.selectedLink ? this.selectedLink.target : ""
      );
      const newSourceLabel =
        prompt("请输入新的起始节点标签", sourceLabel) || sourceLabel;
      const newTargetLabel =
        prompt("请输入新的目标节点标签", targetLabel) || targetLabel;
      const sourceNode = this.nodes.find(
        (node) => node.label === newSourceLabel
      );
      const targetNode = this.nodes.find(
        (node) => node.label === newTargetLabel
      );
