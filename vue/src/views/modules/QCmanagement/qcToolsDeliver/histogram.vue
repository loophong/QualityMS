  ...
  methods: {
    ...
    addData() {
      ...
      if (this.newCategory && !isNaN(value)) {
        ...
        const combined = this.categories.map((category, i) => ({
    ...
    updateStatistics() {
      ...
      // 计算加权平均
      const totalWeightedValue = this.categories.reduce(
        (sum, category, index) => {
          return sum + Number(category) * this.data[index];
        },
        0
      );
      this.weightedMean = this.totalFrequency
        ? (totalWeightedValue / this.totalFrequency).toFixed(2)
        : 0;
